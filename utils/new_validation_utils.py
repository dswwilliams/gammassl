...
@torch.no_grad()
def perform_batch_ue_validation_soft_consistency(
                        val_imgs, 
                        val_labels,
                        model,
                        opt,
    ...
    if opt.frozen_target:
        seg_masks_K_t_unmasked = model.target_seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=None)
    else:
        seg_masks_K_t_unmasked = model.seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=None)


    if opt.use_adaptive_masking:
        # redefine masks if using adaptive masking
        val_masks, _ = get_query_masks(
                            seg_masks_K_t_unmasked, 
                            p=opt.adaptive_masking_p, 
                            return_valid_region_masks=True, 
                            uncertainty_threshold=opt.mask_threshold,
                            query_mask_scaling=opt.query_mask_scaling,
                            query_mask_temp=opt.query_mask_temp,
                            query_mask_region=opt.query_mask_region,
                            )

    # NOTE: only masking query branch
    seg_masks_K_q_masked = model.seg_net.get_query_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=val_masks)
    seg_masks_K_q_unmasked = model.seg_net.get_query_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid)

    segs_K_q_unmasked = torch.argmax(seg_masks_K_q_unmasked, dim=1)

    soft_consistency = get_soft_consistency(seg_masks_K_q_masked, seg_masks_K_t_unmasked)

    bs, h, w = soft_consistency.shape
    uncertainty_maps_ranked = (soft_consistency).view(bs*h*w).argsort(stable=True).argsort(stable=True).view(bs, h, w).float()
    uncertainty_maps_ranked = uncertainty_maps_ranked / uncertainty_maps_ranked.max()       # norm to [0,1]

    ### varying p(certain) and calculating certainty metrics ###
    ue_metrics_ranked, _ = calculate_ue_metrics(
                                segmentations=segs_K_q_unmasked,        # we want to know how the soft consistency mitigates the error of the UNmasked query segmentation
                                labels=val_labels,
                                uncertainty_maps=uncertainty_maps_ranked,
                                max_uncertainty=opt.max_uncertainty,
                                num_thresholds=opt.num_thresholds,
    ...
    return ue_metrics_ranked


@torch.no_grad()
def perform_batch_ue_validation_consistency(
                        val_imgs, 
                        val_labels,
                        val_masks,
                        model,
    ...
    if val_masks_t is None:
        if opt.frozen_target:
            seg_masks_K_t = model.target_seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=val_masks_t)
        else:
            seg_masks_K_t = model.seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=val_masks_t)
        seg_masks_K_t_masked = seg_masks_K_t
        seg_masks_K_t_unmasked = seg_masks_K_t
    else:
        if opt.frozen_target:
            seg_masks_K_t_masked = model.target_seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=val_masks_t)
            seg_masks_K_t_unmasked = model.target_seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=None)

        else:
            seg_masks_K_t_masked = model.seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=val_masks_t)
            seg_masks_K_t_unmasked = model.seg_net.get_target_seg_masks(val_imgs, include_void=False, high_res=True, use_sigmoid=opt.val_with_sigmoid, masks=None)
    ...
    segs_K_t_masked = torch.argmax(seg_masks_K_t_masked, dim=1)
    segs_K_t_unmasked = torch.argmax(seg_masks_K_t_unmasked, dim=1)
    segs_K_q_masked = torch.argmax(seg_masks_K_q_masked, dim=1)
    segs_K_q_unmasked = torch.argmax(seg_masks_K_q_unmasked, dim=1)

    # at this point we've got segs_K_q, segs_K_t, uncertainty_maps_q, uncertainty_maps_t
    # and we want to calculate how well consistency between segs_K_q, segs_K_t approximates the ground truth

    bs, H, W = segs_K_q_masked.shape
    square_masks = val_masks.reshape(-1, H//14, W//14).float()
    square_masks = F.interpolate(square_masks.unsqueeze(1), (H, W), mode='nearest').squeeze(1)      # [bs, H, W]

    ### varying p(certain) and calculating certainty metrics ###
    ue_metrics = calculate_ue_metrics_consistency(
                                        segs_q_masked=segs_K_q_masked,
                                        segs_q_unmasked=segs_K_q_unmasked,
                                        segs_t_masked=segs_K_t_masked,
                                        segs_t_unmasked=segs_K_t_unmasked,
                                        labels=val_labels,
                                        masks=square_masks,
                                        )
    
    ue_metrics["miou"] = calculate_miou(segmentations=segs_K_q_masked, labels=val_labels, num_classes=model.num_known_classes+1)
    return ue_metrics
